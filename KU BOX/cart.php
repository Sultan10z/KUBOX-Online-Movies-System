...
<body>
    ...
    <section class="shopping-cart">
        ...
        <div class="box-container">
            ...
            <?php
            $grand_total = 0;
            $select_cart = $conn->prepare("SELECT cart.*, products.availability FROM `cart` INNER JOIN `products` ON cart.name = products.name WHERE cart.user_id = ?");
            $select_cart->execute([$user_id]);

            if ($select_cart->rowCount() > 0) {
                while ($fetch_cart = $select_cart->fetch(PDO::FETCH_ASSOC)) {
                    $availability = $fetch_cart['availability'];
                    $disabled = ($availability === 'Not Available') ? 'disabled' : '';
                    ...
                    <form action="" method="POST" class="box">
                        ...
                        <?php
                        // Disable the "Proceed to Checkout" button if the availability is "Not Available"
                        ?>
                    </form>
            <?php
                }
        ...
        <div class="cart-total">
            ...
            <a href="checkout.php" class="btn <?= ($grand_total >= 1) ? '' : 'disabled'; ?> <?= $disabled; ?>">proceed to checkout</a>
